...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
  ...
  @Override
  public Result getUserTableSe(int page, int limit, String value) {
    // TODO Auto-generated method stub
    JSONObject jsonObject = new JSONObject();
    // user表总条数
    long total = 0;
    ...
    // mybatis-plus分页查询,page为页码，limit为每页多少条
    Page<User> p = new Page<>(page, limit);
    QueryWrapper<User> wrapper = new QueryWrapper<>();
    // 如果有条件，获取条件参数，把条件加到wrapper中
    if (!value.equals("")) {
      wrapper.like("id", value).or().like("username", value).or().like("password", value);
    }
    IPage<User> iPage = usermapper.selectPage(p, wrapper);
